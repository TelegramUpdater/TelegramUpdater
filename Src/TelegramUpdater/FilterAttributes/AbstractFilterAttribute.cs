...
///<inheritdoc/>
public abstract class AbstractUpdaterFilterAttribute : AbstractFilterAttribute
{
    /// <summary>
    /// Get the inner filter for type <paramref name="requestedType"/> if possible.
    /// </summary>
    protected internal abstract object GetUpdaterFilterTypeOf(Type requestedType);

    ///<inheritdoc/>
    protected internal override object GetFilterTypeOf(Type requestedType)
    {
        return GetUpdaterFilterTypeOf(requestedType.GetGenericArguments()[0]);
    }
}