...
public class UpdaterDataFilter<T> : UpdaterFilter<T>
{
    ...
    protected virtual bool TryGetValue(
        ...
        => Region switch
        {
            ...
            DataRegion.Layer => input.Updater.TryGetLayerItem(new HandlingStoragesKeys.LayerId(input.ScopeId, input.LayerId), key, out value),
    ...
    protected virtual void Remove(UpdaterFilterInputs<T> input, string key)
    {
        switch (Region)
        {
            ...
            case DataRegion.Layer:
                input.Updater.RemoveLayerItem(new HandlingStoragesKeys.LayerId(input.ScopeId, input.LayerId), key);
                break;
        }
    }
